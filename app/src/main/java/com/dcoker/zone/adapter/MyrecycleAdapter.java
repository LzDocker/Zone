...
public class MyrecycleAdapter extends RecyclerView.Adapter<MyrecycleAdapter.ViewHolder> {
    public mineArticleBean mineBean = null;
    ...
    public MyrecycleAdapter(mineArticleBean mineBean,Context context) {
    ...
    public void onBindViewHolder(ViewHolder viewHolder, int position) {
        Article article = mineBean.getData().getArticleList().get(position);
        viewHolder.tv_title.setText(article.getTitle());
        viewHolder.tv_desc.setText("."+article.getLikces()+"赞 ."+article.getComment()+"评");
        Glide.with(context).load(mineBean.getData().getArticleList().get(position).getPicture()).transform(new GlideRoundTransform(context)).placeholder(R.mipmap.ic_launcher).error(R.mipmap.my_nomal).centerCrop().into(viewHolder.imageView);

    }
    //获取数据的数量
    @Override
    public int getItemCount() {
        return mineBean.getData().getArticleList().size();
    }
